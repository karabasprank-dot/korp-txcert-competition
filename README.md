# Korp Promise Receipt & TxCert

**Korp Promise Receipt** is our experimental design for binding a seller’s signed promise to a crypto payment authorization and revealing a specific signed contradiction. Built on the existing TxCert project. AI-assisted and unaudited.
...
A merchant signs acceptance terms. Their digest and signature are committed inside an EIP-3009 nonce. A later merchant-signed response root allows disclosure of one failed equality rule without publishing the unrelated field values. The candidate contribution is the nonce-based acceptance commitment; existing signatures, receipts and selective disclosure are acknowledged prior art. Global novelty and patentability are not established.
...
## Reproduce
Extract `korp-txcert-source.zip` (source structure preserved). Node 24 and Python 3. Run `npm install --ignore-scripts`, `npm run check`, `npm run demo:chain` and `npm run demo:competition`. The snapshot includes 194 passing tests, live Worker/UI, testnet payment verification script, and public evidence.
